...
export async function homePage(container) {
    ...
    const getFinalPrice = (p) => typeof p.discount === "number" && p.discount > 0 ? p.price * (1 - p.discount / 100) : p.price;
    ...
    const getFilteredProducts = () => {
        ...
        return products
            ...
            const matchesCategories = selectedSet.size === 0 || p.categories.some((c) => selectedSet.has(c));
    ...
    const renderCategories = (cats) => {
        categoriesContainer.innerHTML = cats
          ...
          <label class="category-chip">
            <input type="checkbox" class="category-chip-input" data-category="${cat}" ${checked ? "checked" : ""} />
    ...
    catalogRoot.addEventListener("click", async (event) => {
        ...
        if (!(target instanceof HTMLElement))
            return;
        const btn = target.closest('button[data-add-to-basket]');
        if (!btn)
            return;
        if (!state.user) {
            await navigate("/auth");
        ...
        const qtyInput = card?.querySelector('input[data-quantity]');
